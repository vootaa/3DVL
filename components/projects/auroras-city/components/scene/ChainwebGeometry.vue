...
onUnmounted(() => {
    ...
    if (chainwebGeometry.value) {
        ...
        chainwebGeometry.value.rings.forEach((geo: { dispose: () => any }) => geo.dispose())
        chainwebGeometry.value.nodes.forEach((geo: { dispose: () => any }) => geo.dispose())
        chainwebGeometry.value.sameChainConnections.forEach((geo: { dispose: () => any }) => geo.dispose())
        chainwebGeometry.value.crossChainConnections.forEach((geo: { dispose: () => any }) => geo.dispose())
    }
...
<template>
    <TresGroup v-if="chainwebGeometry" :position="position" :scale="scale" :rotation="currentRotation">
        <!-- Ring Geometries -->
        <TresMesh v-if="showRings" v-for="(ringGeo, index) in chainwebGeometry.rings" :key="`ring-${index}`"
            :geometry="ringGeo" :material="ringMat" />

        <!-- Node Geometries -->
        <TresMesh v-if="showNodes" v-for="(nodeGeo, index) in chainwebGeometry.nodes" :key="`node-${index}`"
            :geometry="nodeGeo" :material="nodeMat" />

        <!-- Same Chain Connections -->
        <TresLine v-if="showSameChainConnections"
            v-for="(connectionGeo, index) in chainwebGeometry.sameChainConnections" :key="`same-chain-${index}`"
            :geometry="connectionGeo" :material="connectionMat" />

        <!-- Cross Chain Connections -->
        <TresLine v-if="showCrossChainConnections"
            v-for="(connectionGeo, index) in chainwebGeometry.crossChainConnections" :key="`cross-chain-${index}`"
            :geometry="connectionGeo" :material="connectionMat" />
    </TresGroup>
</template>